...
def open_quiz():
    """Ouvre une fenêtre de quiz FR → EN."""
    ...
    instruction_label = tk.Label(content_frame, text="Traduisez en anglais :", font=("Arial", 12), bg="#eef5ff")
    ...
    def verifier_reponse():
        ...
        if isinstance(correct, (list, tuple)) and correct:
            ...
            correct_answer = str(correct).lower()

        if user == correct_answer:
            score += 1
            result_label.config(text="✅ Bonne réponse !", fg="#0a8f3c")
        else:
            result_label.config(text=f"❌ Faux ! Réponse : {correct_answer}", fg="#ce2f2f")
